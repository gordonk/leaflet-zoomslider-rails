            ...
            for (var i = 0; i < data.features.length; i++) {
                ...
                if (!onMap) {
                    ...
                    // Convert GeoJSON to Leaflet vector (Point, Polyline, Polygon)
                        var dataOption = this.options.includeDataOption ? data.features[i] : null;
                        var vector_or_vectors = this._geoJsonGeometryToLeaflet(data.features[i].geometry, this._getFeatureVectorOptions(data.features[i]), dataOption);
